...
int main() {
    ...
    std::map<std::string, uint64_t> resuts;
    resuts = toMateTo("r4rk1/1pp1qppp/p1np1n2/2b1p1B1/2B1P1b1/P1NP1N2/1PP1QPPP/R4RK1 w - - 0 10");
    for (const auto& [move, toMateTo_eval] : resuts) {
        std::cout<< "\n the move: " << move <<  " has an eval of: " << toMateTo_eval <<  std::flush; 
    }
