...
const std::string FEN_TEST_POSITIONS[AMOUNT_TEST_POS] = {
...
void move_gen_speed_testing(int depth){
    ...
    for(int i = 0; i < AMOUNT_TEST_POS; i++){
        setup_fen_position(board, FEN_TEST_POSITIONS[i]);

        count += perft(&board, depth);

        std::cout << "\nPERFT tested: " << i << std::flush;
    }
    ...
    std::cout << "\nNodes: " << count;
    std::cout << "\nNodes per second: " << count / seconds << "\n";
}